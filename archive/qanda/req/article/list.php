...
<div class="row m-2">
 <div class="col-sm-2">
  <div class="btn btn-primary" onClick="jumpapi('req/article/postedit.php')">投稿する</div>
 </div>
 <div class="col-sm-7">
...
print<<<EOF
<div class="artfrm" articleid="{$art['ARTICLE_ID']}" mode="{$mode}">
 <div class="arthead">{$username} が{$postdt} に投稿</div>
 <div class="arttitle">{$art['TITLE']}</div>
 <div class="artfoot">{$postlikecnt} イイね</div>
</div>
EOF;

}
if (count($retinfo['articlelist']) == 0) {
	print('<div class="row m-2"><div class="col-sm-12">1件も記事はありません</div></div>');
}

// ページセクション
print('<div class="row m-2">');
// 前送り
if ($page <= 0) {
	printf('<div class="col-sm-4 text-right"><span class="btn btn-link disabled ">← 前</span></div>');
}
else {
	$urlstr = sprintf("jumpapi('req/article/list.php?page=%d')", $page - 1);
	printf('<div class="col-sm-4 text-right"><span class="btn btn-link" onclick="%s">← 前</span></div>', $urlstr);
}
// ページとページ数
...
if ($page >= $retinfo['MAXPAGE']) {
	printf('<div class="col-sm-4 text-left"><span class="btn btn-link disabled">次 →</span></div>');
}
else {
	$urlstr = sprintf("jumpapi('req/article/list.php?page=%d')", $page + 1);
	printf('<div class="col-sm-4 text-left"><span class="btn btn-link" onclick="%s">次 →</span></div>', $urlstr);
}
print('</div>');
