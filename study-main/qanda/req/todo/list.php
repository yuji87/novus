...
<!-- submitだと javascriptによる入力チェック処理が呼ばれないので divのbtn とした
  <input type="submit" class="btn btn-primary" onClick="onAddToDo();" value="ToDoリスト" />
-->
...
foreach ($retinfo['activelist'] as $active) {
    ...
    echo '<tr><td>' . $idx . '</td><td>' . $active['TITLE'] . '</td><td>' . $active['REMIND_DATE'] . '</td><td>';
    echo '<span class="btn btn-link done" todoid="' . $active['TODO_ID'] . '">Done</span>';
    echo '<span class="btn btn-link edit" todoid="' . $active['TODO_ID'] . '" todotitle="' . $active['TITLE']
        . '" tododt="' . $active['REMIND_DATE'] . '">Edit</span>';
    echo '<span class="btn btn-link delete" todoid="' . $active['TODO_ID'] . '">Delete</span>';
    echo '</td></tr>';
    $idx++;
}
if (count($retinfo['activelist']) == 0) {
    echo '<tr><td colspan="4">1件もありません</td></tr>';
}
?>
</table>
...
foreach ($retinfo['finlist'] as $fin) {
    ...
    echo '<tr><td>' . $idx . '</td><td>' . $fin['TITLE'] . '</td><td>' . $fin['REMIND_DATE'] . '</td><td>';
    echo '<span class="btn btn-link return" todoid="'. $fin['TODO_ID'] . '">Return</span>';
    echo '<span class="btn btn-link edit" todoid="' . $fin['TODO_ID'] . '" todotitle="' . $fin['TITLE']
        . '" tododt="' . $fin['REMIND_DATE'] . '">Edit</span>';
