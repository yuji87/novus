...
<div class="row m-2 ml-5 pl-5 col-sm-12">
  ...
  if ($page <= 0) {
    ...
    $urlstr = sprintf("%sarticle/home.php?page=%d", DOMAIN, 0);
  ...
  if ($page <= 0) {
    ...
    $urlstr = sprintf("%sarticle/home.php?page=%d", DOMAIN, $page - 1);
    printf('<div class="col-sm-1 text-right"><a class="btn btn-link" href="%s">&lt;</a></div>', $urlstr);
  }
  // ページボタン
  $start = $page - 3;
  ...
  for ($i = $start; $i <= $end; $i++) {
    if ($i == $page) {
      printf('<div class="col-sm-1 cur"><a class="btn btn-primary  disabled">%d</a></span></div>', $i + 1);
    }
    else {
      $urlstr = sprintf("%sarticle/home.php?page=%d", DOMAIN, $i);
      printf('<div class="col-sm-1"><a class="btn btn-light" href="%s">%d</a></span></div>', $urlstr, $i + 1);
    }
  }
  // 次送り 一つ先
  if ($page >= $retinfo['MAXPAGE']) {
    ...
    $urlstr = sprintf("%sarticle/home.php?page=%d", DOMAIN, $nextpage);
  ...
  if ($page >= $retinfo['MAXPAGE']) {
    ...
    $urlstr = sprintf("%sarticle/home.php?page=%d", DOMAIN, $retinfo['MAXPAGE']);
    printf('<div class="col-sm-1 text-left"><a class="btn btn-link" href="%s">&gt;&gt;</a></div>', $urlstr);
  }
  ?>
</div>
...
?>
  ...
  $('#searcharticle').change(function() {
    ...
    jumpapi('article/home.php?page=' + <?php echo $page; ?> + '&searchtext=' + encodeURIComponent(txtdata));
  });
</script>
